...
function Orders() {
  ...
  if (isLoading) return <h1>Loading...</h1>;
    ...
    return (
      <div className="w-full flex flex-col justify-center items-center h-[80vh]">
        ...
        <h2 className="text-3xl font-medium text-gray-600 tracking-wide">
          No Orders Yet
        </h2>
      </div>
    );
  ...
  function filterOrdersByStatus() {
    if (orderFilter.orderStatus === "all orders") return;
    filteredOrders = orders.filter((order) => {
      if (
        order.orderItems[0].status.currentStatus.toLowerCase() ===
        orderFilter.orderStatus.toLowerCase()
      )
        return true;
      else return false;
    });
  ...
  function filterOrdersByTime() {
    if (orderFilter.time === "all time") return;
    filteredOrders = filteredOrders.filter((order) => {
      const orderDate = new Date(order.createdAt);
      const today = new Date();
      let prevDate;
      switch (orderFilter.time) {
        case "30 days":
          prevDate = new Date(
            today.getFullYear(),
            today.getMonth() - 1,
          ...
          break;
        case "6 months":
          prevDate = new Date(today.getFullYear(), today.getMonth() - 5);
          break;
        case "year":
          prevDate = new Date(today.getFullYear() - 1, today.getMonth());
          break;
      }
      if (today >= orderDate && orderDate >= prevDate) return true;
      else return false;
    });
  }
  ...
  console.log("orders for", email, filteredOrders);
  const orderItems = [];
  for (let i = 0; i < filteredOrders.length; i++) {
    ...
    for (let j = 0; j < order.orderItems.length; j++) {
      ...
      orderItems.push(
        <OrderItem
          key={element._id}
          order={{ order: element, _id: order._id }}
        />
      );
    }
  }
